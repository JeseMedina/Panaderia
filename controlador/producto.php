...
switch ($_GET["op"]){
    case 'guardaryeditar':
        if (empty($idproducto)){
            $rspta=$producto->insertar($idrubro,$codigo,$nombre,$stock,$uMedida,$precioCosto,$precioVenta);
            echo $rspta ? "Artículo registrado" : "Artículo no se pudo registrar";
        }
        else {
            $rspta=$producto->editar($idproducto,$idrubro,$codigo,$nombre,$stock,$uMedida,$precioCosto,$precioVenta);
        ...
        while ($reg=$rspta->fetch_object()){
            $data[]=array(
                "0"=>($reg->condicion)?'<button class="btn btn-warning" onclick="mostrar('.$reg->idproducto.')"><i class="fa fa-pencil"></i></button>'.
                    ...
                    '<button class="btn btn-primary" title="Editar producto disabled"><i class="fa fa-pencil"></i></button>'.
